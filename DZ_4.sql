...
SELECT * from cats WHERE name not in ("Murzik", "Zuza");
SELECT shops.shopname, GROUP_CONCAT(cats.name) as cats_name FROM cats
RIGHT JOIN shops
ON shops.id = cats.shops_id
GROUP BY shopname
HAVING cats_name not like "%Murzik%" AND cats_name not LIKE "%Zuza%" or cats_name is NULL;
...
(
	...
	ord_datetime DATETIME NOT NULL, -- 'YYYY-MM-DD hh:mm:ss'
	ord_an INT,
...
SELECT  Orders.ord_datetime, Analysis.an_name, Analysis.an_price FROM Analysis
RIGHT JOIN Orders
ON an_id = ord_an
